...
3. **Route cutover** — routes/companion READ from `store.*`; writes go through `database/writes.py` (dual-write SQLite + Chroma), and `reminders`/`profile` mirror their JSON writes to SQLite (their readers stay JSON). Response shapes unchanged. Write ordering: creates SQLite-first + Chroma best-effort; deletes Chroma-first. Semantic retrieval (`fetch_revelant_memories`, `/briefing`) + face matching stay on Chroma. **Eval+pytest match baseline.** ✅ Done 2026-05-30 (44 tests green; verified live).
4. **Episodic/semantic typing + provenance** — typing done in Step 3 (`writes.record_memory`); provenance **surfaced** in reads via a JOIN (`added_by`/`added_at`). Chroma-metadata denormalization dropped (reads come from SQLite). ✅ Done 2026-05-30.
5. **Hybrid retrieval + rerank** — `database/retrieval.py` (vector → join → filter → rerank[sim+keyword+decay-by-kind+frequency] → top-k → bump use_count); companion uses it; pure `_score` unit-tested; graceful `[]` on failure. ✅ Done 2026-05-30 (53 tests green, eval no-regression). **Open:** the recall@k *win* needs a richer seed than the current ~5 facts + weight tuning — flagged follow-up.
